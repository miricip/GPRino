...
#define PRI       320                 //PRI in ms for 3.125 scans per second
...
const word delimiter = 0;               	//delimiter
...
void loop()
{
  ...
  while (digitalRead(button_start))
  {
    ...
    if (!digitalRead(button_down))
    {
      ...
      xpos = tft.width() / 2; // Middle of screen
      ypos = (tft.height() / 2) - tft.fontHeight(GFXFF); // Middle of screen
      String temp_str = "REF LVL = " + String(long(max_amplitude));
      char text[16];
      temp_str.toCharArray(text, 16);
      tft.drawString(text, xpos, ypos, GFXFF);
      delay(500);
    }
    else if (!digitalRead(button_up))
  ...
  delay(50);
  // Wait until Start button is unpressed
  while (!digitalRead(button_start))
  {
    check_displ_batlevel();
  }
  ...
  while (digitalRead(button_start))
  {
    ...
    if (current_time - previous_time >= PRI)  // if it's time to scan
    {
      previous_time = current_time;
      // If screen is full, delete and start again
      if ((scan_index % no_res_cells_horiz) == 0)
      {
        word screen_index = scan_index / no_res_cells_horiz;
        tft.fillScreen(BLACK);
        Graph(tft, orig_x, orig_y, graph_width, graph_height, screen_index * max_dist, (screen_index + 1) * max_dist, dist_step, min_depth, max_depth, depth_step, "GPR", "Distance [m]", "Depth [m]");
        show_card_status();
      }
      check_displ_batlevel();
      ...
      //Draw one column
      //double maximum=0; // FFT maximum (for calibrating display)
      for (i = 0; i < no_res_cells_vert; i++)
      {
        if (i < 60) {
          ...
          depth_corrected_amplitude = real[i] * 4.482;
        }
      ...
      //Serial.print("Maximum="); Serial.println(maximum); // display FFT maximum (for calibrating display)
      scan_index++;
      //Serial.println(millis() - previous_time); // display scan duration (for setting minimum PRI)
    }
  }
  delay(50);
  // Wait until Start button is unpressed
  ...
  delay(50);
  //Store on SD card the delimiter (a scan all zeroes)
  if (card)
  {
    file = SD.open("Data.gpr", FILE_WRITE);
    for (i = 0; i < no_of_samples; i++)
    {
      file.write(highByte(delimiter));
      file.write(lowByte(delimiter));
      //file.flush();
    }
    file.close();
  }
