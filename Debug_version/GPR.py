...
if image_rows > 128 * interp_order_depth + 1:
    image_rows = 128 * interp_order_depth + 1
...
iteration = 1
if save_data=='y':
    data_for_save = numpy.zeros((128 * interp_order_depth + 1, 1)) #create blank data matrix for saving
    for i in range(128 * interp_order_depth):    
...
while True:
    if ser.in_waiting > 0:    
        ...
        if debugging_mode==1:
            ...
            column = numpy.absolute(numpy.fft.rfft(s_array, 256 * interp_order_depth))        #compute fft magnitude
            if save_data=='y':
                column_conc=numpy.concatenate((numpy.array([iteration*step]), column), axis=0)
                data_for_save=numpy.hstack((data_for_save,numpy.transpose(numpy.array([column_conc[0:(128 * interp_order_depth + 1)]]))))  #add as new last column
            for i in range(128 * interp_order_depth + 1):
